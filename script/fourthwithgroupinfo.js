(async () => {
  try {
    ...
    async function createAndLogGroups(participantCount) {
      ...
      await communityGovernance.methods.createGroups(participants).send({ from: accounts[0], gas: 3000000 });
      
      const groupCount = await communityGovernance.methods.getGroupCount().call();
      console.log(`Total groups created: ${groupCount}`);
      
      for (let i = 0; i < groupCount; i++) {
        const groupMembers = await communityGovernance.methods.getGroupMembers(i).call();
